                                                    ...
                                                    <tbody>
                                                        @foreach ($orders as $order)
                                                            <tr>
                                                                <td>#{{ $order->id }}</td>
                                                                <td>
                                                                    {{-- limit text only 20 with str limit --}}
                                                                    {{ Str::limit($order->order_items->first()->product->name, 40) }}
                                                                    @if ($order->order_items->count() > 1)
                                                                        and {{ $order->order_items->count() - 1 }}
                                                                        item more
                                                                    @endif
                                                                </td>
                                                                <td>{{ $order->status }}</td>
                                                                <td>{{ $order->getFormattedTotalPriceAttribute() }}
                                                                </td>
                                                                <td>
